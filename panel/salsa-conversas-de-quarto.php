...
<div class="app-container app-theme-white body-tabs-shadow fixed-header fixed-sidebar">
  ...
  <tbody>
    <?php
                            $sql = "SELECT * FROM chatlogs_room order by timestamp DESC LIMIT 20";
      $query = mysqli_query($conn, $sql) or die(mysqli_error($conn));
      while($row = $query->fetch_assoc())
      {
        $sql2 = "SELECT * FROM users WHERE id='".$row['user_from_id']."'";
    ...
    <tr>
      ...
      <td><?php echo date('d/m/Y', $row['timestamp']) . ' às ' . date('H:i:s', $row['timestamp']) ?></td>
